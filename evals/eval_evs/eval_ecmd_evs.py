...
from utils.load_utils import load_gt_us, ecmd_evs_iterator
    ...
    for i, scene in enumerate(scenes):
        ...
        for trial in range(trials):
            ...
            traj_est, tstamps, flowdata = run_voxel(datapath_val, config, net, viz=viz, 
                                          iterator=ecmd_evs_iterator(datapath_val, side=side, stride=stride, timing=timing, H=H, W=W),
                                          timing=timing, H=H, W=W, viz_flow=viz_flow)

            # load  traj（这应该是获取gt trajectory的值,从txt文件中读取,注意读入的为秒）
            tss_traj_s, traj_hf = load_gt_us(os.path.join(datapath_val, f"GT_{scene.lower()}.txt"))
...
if __name__ == '__main__': 
    ...
    print("Running ECMD dataset with config...")
    print(cfg) 
