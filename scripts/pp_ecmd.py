...
def process_dirs(indirs, DELTA_MS=None):
    for indir in indirs: 
        ...
        print(f"\n\n ECMD: Undistorting {seq} evs & rgb & IMU & GT")#处理某个序列的数据

        inbag = os.path.join(indir, f"../{seq}.bag")#获取bag文件的路径
        bag = rosbag.Bag(inbag, "r")#读取bag文件

        imagetopic='/DAVIS346_left/image'
        ...
        intrinsics = [251.94813034394284, 251.0541106928348, 172.20478771290288, 135.648676738234, 
                    -0.39463069281995433, 0.16676125489995194, -0.00046987660901625847, 0.001958270184109082]
        ...
        f = open(os.path.join(indir, f"calib_undist_davis346.txt"), 'w')
        ...
        # 获取GT pose（注意ECMD GT pose在额外的文件夹中）
        ...
        #读取events数据的起始时间
        loweventtopic='/DAVIS346_left/events'
        if has_outlier:
        ...
        # ! 下面是读取dcxplorer及保存数据的代码
        higheventtopic='/DVXplorer_left/events'
        ...
        # 高分辨事件相机的内参
        dvsplorer_intrinsics = [547.2703829559849, 545.4540498341149, 320.2935455165061, 241.843126203522, 
                    -0.40575879021628114, 0.20616097747818823, -0.002622678645791178, -0.0008709741411368295]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PP ECMD data in dir")
    parser.add_argument(
        "--indir", help="Input image directory.", default=""
    )
    args = parser.parse_args()

    record_file = os.path.join(args.indir, "record_processed_ecmd.txt")
    ...
    print(f"Finished processing all ECMD scenes")
